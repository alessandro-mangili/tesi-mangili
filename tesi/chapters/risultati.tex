\chapter{Risultati}
    \label{chap:risultati}
        \section{RetDec + Klee}
        \label{subsec:risultati_retdec+klee}
            ...
            \label{subsec:decompilazione_incompleta}
...
\begin{clangcode}[fontsize=\footnotesize]
if (res == FR_OK) {
    f_close(&file_struct);
    magic[7] = '\0';
...
\end{clangcode}
        \section{Angr}
        \label{subsec:risultati_angr}
        \subsection{Esplorazione dei percorsi}
        \label{subsec:risultati_esplorazione_percorsi}
        ...
        \input{tesi/tables/constraints_magic}
...
\end{bashcode}
    ...
    \label{subsec:risultati_breakpoint_memoria}
...
\end{bashcode}
    ...
    \label{subsec:vulnerabilitá_integer_underflow}
        I vincoli osservati sia nell'esplorazione dei percorsi (~\Cref{subsec:risultati_esplorazione_percorsi}) sia nel breakpoint in memoria (~\Cref{subsec:risultati_breakpoint_memoria}) hanno mostrato che il limite superiore ammesso per \texttt{filinfo.fsize} è nell'ordine dei \texttt{0xffff2000} byte, un valore ($\sim$4\,GB) del tutto incompatibile con la reale capacità della memoria Flash del dispositivo (dell'ordine del MB).
...
\end{clangcode}
        ...
        \input{tesi/tables/flash_size_data_register}
...
\end{bashcode}
    ...
    \label{subsec:vulnerabilitá_identificate}
        ...
        \paragraph{Validazione dell'header insufficiente}
            ...
            contenuto inizi con la sequenza di byte determinata in \Cref{subsec:angr} (Tabella~\ref{tab:magic-constraints}):
        ...
        \paragraph{Controllo limitato sul nome del file}
            ...
            del bootloader (\Cref{subsec:funzioni_critiche}), si limita alla corrispondenza esatta con 
        ...
        Come dimostrato in \Cref{subsec:vulnerabilitá_integer_underflow}, il valore del registro 
...
\label{chap:conclusioni}
    ...
    \label{subsec:limiti_strumenti}
        ...
        \label{subsec:efficacia_angr}
            I risultati ottenuti con Angr, presentati nelle \Cref{subsec:risultati_retdec+klee}--\Cref{subsec:risultati_angr}, 
            ...
            tramite riscrittura manuale del codice intermedio (\Cref{subsec:stub_periferiche_klee}).
    ...
    \label{subsec:sviluppi_futuri}
        ...
        Buona parte del lavoro manuale descritto nel \Cref{chap:metodologia}(identificazione 
        ...
        timeout, pattern osservato manualmente in questo lavoro (\Cref{subsec:intercettazione_uart}) 
        ...
        \texttt{\_flash\_size\_data\_register} (\Cref{subsec:vulnerabilitá_integer_underflow}), così da 
        convalidare sul campo quanto osservato nel modello.
    

    \section{Considerazioni finali}
    \label{subsec:considerazioni_finali}
